...
@test "Darwin install retries a transient bootstrap failure" {
  setup_cao_service_test Darwin
  ...
  [[ "$(manager_call_count bootstrap)" -eq 2 ]]
}

@test "Darwin runtime refresh kickstarts without unloading the label" {
  setup_cao_service_test Darwin
  run_service install
  assert_success
  ...
  [[ "$(manager_call_count bootstrap)" -eq 1 ]]
  [[ "$(manager_call_count bootout)" -eq 0 ]]
  [[ "$(manager_call_count kickstart)" -eq 1 ]]
}
